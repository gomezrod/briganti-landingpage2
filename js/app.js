...
function startLoader() {
  ...
  let progressFill = document.querySelector(".loader__progress-fill");
  let currentValue = 0;
  function updateCounter() {
    if (currentValue < 100) {
      ...
      let delay = Math.floor(Math.random() * 60) + 25;
...
$(function() {
  ...
  // Build gallery array once on DOM ready
  var pswpGallery = [];
  ...
  if (galleryItems.length > 0) {
    ...
    galleryItems.forEach(function(item) {
      var sizeData = item.getAttribute('data-size');
      if (sizeData && sizeData.includes('x')) {
        var parts = sizeData.split('x');
        pswpGallery.push({
    ...
    // Attach click handler to gallery container
    var liveGalleryGrid = document.querySelector('.live-gallery-grid');
    if (liveGalleryGrid) {
      liveGalleryGrid.addEventListener('click', function(e) {
        if (e.target.closest('.gallery-item')) {
          e.preventDefault();
          var clickedItem = e.target.closest('.gallery-item');
          var index = Array.from(galleryItems).indexOf(clickedItem);
          
          if (index >= 0 && pswpElement) {
            var options = {
              index: index,
              history: false,
              focus: false,
            ...
            var lightBox = new PhotoSwipe(pswpElement, PhotoSwipeUI_Default, pswpGallery, options);
            lightBox.init();
          }
        }
      });
